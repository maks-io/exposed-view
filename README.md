...
| `addAttributes`     | no       | `true`      | boolean             | This allows you to add the attributes `data-ev-x`, `data-ev-y`, `data-ev-width` and `data-ev-height` to every container when used on web (via `react-native-web`). In other words, this exposes the position and dimensions of containers on the web, which can be helpful for debugging or e2e testing, for instance. Of course this properties are not available if no measurements are available. |
